...
In this work, we study the \emph{``ground truth''} errors made by LLMs in the text-to-SQL task looking at both fine-tuned models and general-purpose models optimized for code generation.
We observe that errors can be grouped into five error categories, two of which are amenable to simple symbolic fixes.
Importantly, we find that for many queries, correct predictions do not rank near the top of the LLMs likely predictions, implying that \emph{constrained decoding alone is insufficient} to guide synthesis on hard problems.
