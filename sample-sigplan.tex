...
Existing approaches to improve accuracy generally rely on post-hoc verification, that fails to constrain unproductive path exploration, or (FIXME maybe say symbolic fixes for program synthesis approaches) constrained decoding techniques that rely on learned distributions leaning towards correct programs.
In this work, we study the mistakes LLMs make when synthesizing SQL queries.
We observe that the mistakes can be grouped into \wip{<x>} error categories, \wip{<y>} of which are amenable to symbolic fixes.
